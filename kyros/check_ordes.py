...
class CheckOrders(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(name="check_orders", description="Check all submitted orders")
    ...
    async def check_orders(self, ctx):
        ...
        orders = []
        for filename in os.listdir("orders/kyros"):
            if filename.endswith(".json"):
                filepath = os.path.join("orders/kyros", filename)
...
async def setup(bot):
    await bot.add_cog(CheckOrders(bot))
